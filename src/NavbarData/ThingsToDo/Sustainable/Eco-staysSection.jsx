...
const EcoStaysSection = ({ 
  ...
  const totalPrinciples = ecoStaysPrinciples.items.length;
  ...
  // Breadcrumb Component (matching other sections)
  const Breadcrumb = () => (
    <div className="w-full bg-gradient-to-r from-[#3BB0C1] to-[#48abbc] text-white py-3 px-6 md:px-10 flex items-center gap-2 text-sm font-medium shadow-md">
      ...
      <span className="text-white/60 text-xs">›</span>
      <span className="opacity-95 font-semibold">Eco‑stays & Homestays</span>
    </div>
  );

  // Principle Carousel Controls
  ...
  return (
    <div className="bg-white min-h-screen">
      ...
      <div className="relative h-[65vh] md:h-[75vh] w-full overflow-hidden">
        <img 
          src={ecoStaysHero.backgroundImage || "https://images.unsplash.com/photo-1596402184320-417e7178b2cd?w=1400&q=80"} 
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-6 md:p-10 border-t-4 border-[#48abbc]">
          ...
          <section className="mb-20">
            ...
            <div className="relative">
                ...
                <div className="flex-1 overflow-hidden">
                  <div className="flex flex-col md:flex-row bg-gray-50 rounded-2xl overflow-hidden shadow-md">
                    <div className="md:w-1/2 h-64 md:h-auto overflow-hidden">
                      <img 
                        src={ecoStaysPrinciples.items[activePrinciple].image} 
                        alt={ecoStaysPrinciples.items[activePrinciple].alt}
                        className="w-full h-full object-cover"
                      />
                    </div>
                    <div className="md:w-1/2 p-6 md:p-8 flex flex-col justify-center">
                      <h3 className="text-xl md:text-2xl font-serif font-bold text-[#48abbc] mb-3">
                        {ecoStaysPrinciples.items[activePrinciple].title}
                      </h3>
                      <p className="text-gray-600 leading-relaxed">
                        {ecoStaysPrinciples.items[activePrinciple].description}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex justify-center gap-2 mt-5">
                {ecoStaysPrinciples.items.map((_, idx) => (
                  <button
                    key={idx}
          ...
          <section className="mb-20">
            ...
            <div className="space-y-12">
              {featuredEcoStays.map((stay, idx) => (
                <div key={stay.id} className={`flex flex-col ${idx % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'} gap-6 bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-100`}>
                  <div className="md:w-1/2 h-64 md:h-auto overflow-hidden">
                    <img src={stay.image} alt={stay.alt} className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" />
                  </div>
                  <div className="md:w-1/2 p-6 md:p-8">
                    <span className="text-4xl font-black text-[#48abbc]/20">0{stay.id}</span>
                    <h3 className="text-2xl font-serif font-bold text-gray-800 mt-2 mb-1">{stay.name}</h3>
                    <p className="text-sm font-medium text-[#48abbc] mb-3">{stay.tagline}</p>
                    <p className="text-gray-600 text-sm leading-relaxed mb-5">{stay.description}</p>
                    {stay.readMore && (
                      <div className="flex gap-3 items-start p-3 bg-gray-50 rounded-xl">
                        <img src={stay.readMore.image} alt="" className="w-16 h-16 rounded-lg object-cover" />
                        <div>
                          <h4 className="font-bold text-sm">{stay.readMore.title}</h4>
                          <p className="text-xs text-gray-500 mt-1">{stay.readMore.description}</p>
                          <span className="text-[10px] text-gray-400 mt-1 block">{stay.readMore.readTime} read</span>
                        </div>
                      </div>
                    )}
                  </div>
          ...
          <section className="mb-20">
            <h2 className="text-2xl md:text-3xl font-serif font-bold text-gray-800 mb-8 border-l-8 border-[#48abbc] pl-6">
              {planYourStay.heading}
            </h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {planYourStay.cards.map((card, idx) => (
