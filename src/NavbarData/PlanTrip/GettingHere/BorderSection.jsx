...
import { BorderData } from '../../../Data/PlanTrip/GettingHere/BorderData';
import { FaPlus, FaMinus } from "react-icons/fa";

const BorderSection = ({ onBack }) => {
  const [openAccordion, setOpenAccordion] = useState({});
  const sectionRefs = useRef({});

  const scrollToBorder = (id) => {
    const el = sectionRefs.current[id];
    if (el) el.scrollIntoView({ behavior: 'smooth', block: 'start' });
  };

  const toggleAccordion = (borderId, index) => {
    const key = `${borderId}-${index}`;
    ...
    <div className="bg-white min-h-screen">
      ...
      <div className="bg-[#3ab0ba] text-white py-3 px-6 md:px-10 text-sm font-medium sticky top-0 z-[60] shadow-md">
        <div className="max-w-7xl mx-auto flex items-center gap-2 flex-wrap">
          ...
          <span>&gt;</span>
          <span className="opacity-80">Border crossings</span>
        </div>
      </div>

      {/* 2. HERO IMAGE + BORDER NAVBAR overlaid */}
      <div className="relative w-full h-[500px] md:h-[620px]">
        <img
          src={BorderData.hero.image}
          alt="Border crossings hero"
          className="w-full h-full object-cover"
        />

        {/* Border navbar on top of hero */}
        <div className="absolute top-0 left-0 w-full bg-black/40 backdrop-blur-md py-4 border-b border-white/10">
          <div className="max-w-7xl mx-auto px-6 flex gap-8 md:gap-12 justify-start lg:justify-center overflow-x-auto scrollbar-hide">
            {BorderData.borders.map(border => (
              <button
                key={border.id}
                onClick={() => scrollToBorder(border.id)}
      ...
      <div className="relative z-10 bg-white -mt-12 mx-6 md:mx-14 lg:mx-24 shadow-2xl pb-20">
        ...
        {BorderData.borders.map((border) => (
          <section
            key={border.id}
            id={border.id}
            ref={el => sectionRefs.current[border.id] = el}
          ...
          >
            ...
            <p className="text-xs text-gray-400 mb-10 flex items-center gap-1">
              <span>📷</span>
              <span>{border.name}, India</span>
            </p>

            <div className="border-t border-gray-200">
              {border.details.map((detail, idx) => {
                const key = `${border.id}-${idx}`;
                const isOpen = openAccordion[key];

                return (
                  <div key={idx} className="border-b border-gray-200">
                    <button
                      onClick={() => toggleAccordion(border.id, idx)}
