...
import { DomesticData } from '../../../Data/PlanTrip/GettingAround/DomesticData';

const DomesticSection = ({ onBack }) => {
  ...
  const toggleAccordion = (cityId, index) => {
    ...
    <div className="bg-white min-h-screen">
      ...
      <div className="bg-[#3ab0ba] text-white py-3 px-6 md:px-10 text-sm font-medium sticky top-0 z-[60] shadow-md">
        <div className="max-w-7xl mx-auto flex items-center gap-2 flex-wrap">
          ...
          <span>&gt;</span>
          <span className="opacity-80">Domestic flights</span>
        </div>
      </div>

      {/* 2. HERO IMAGE + CITY NAVBAR overlaid */}
      <div className="relative w-full h-[500px] md:h-[620px]">
        <img
          src={DomesticData.hero.image}
          alt="Domestic flights hero"
          className="w-full h-full object-cover"
        />

        {/* City navbar on top of hero */}
        ...
        {DomesticData.cities.map((city) => (
          ...
          >
            ...
            <p className="text-xs text-gray-400 mb-10 flex items-center gap-1">
              <span>📷</span>
              <span>{city.name}, India</span>
            </p>

            <div className="border-t border-gray-200">
              {city.details.map((detail, idx) => {
                const key = `${city.id}-${idx}`;
                const isOpen = openAccordion[key];

                return (
                  <div key={idx} className="border-b border-gray-200">
                    <button
                      onClick={() => toggleAccordion(city.id, idx)}
